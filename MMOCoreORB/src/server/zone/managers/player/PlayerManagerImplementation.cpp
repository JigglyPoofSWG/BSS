...
void PlayerManagerImplementation::sendPlayerToCloner(CreatureObject* player, uint64 clonerID, int typeofdeath) {
	...
	if (ghost->getFactionStatus() != FactionStatus::ONLEAVE && cbot->getFaction() == 0 && !player->hasSkill("force_rank_dark_novice") && !player->hasSkill("force_rank_light_novice")) {
		ghost->setFactionStatus(FactionStatus::ONLEAVE);
	} else if (player->hasSkill("force_rank_dark_novice") || player->hasSkill("force_rank_light_novice")) {
