...
int ForceShrineMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) const {
	...
	if (!creature->hasSkill("force_title_jedi_rank_03") && creature->hasSkill("force_title_jedi_rank_02") && (ghost->getSpentJediSkillPoints()) >= 205) {
		ManagedReference<PlayerObject*> ghost = creature->getPlayerObject();
		...
		if (creature->isRebel()){
			...
			ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
			box->setPromptTitle("@jedi_trials:knight_trials_title"); // Jedi Council Battle Trials
			...
			if (inventory->transferObject(lightRobe, -1)) {
				inventory->broadcastObject(lightRobe, true);
			} else {
				lightRobe->destroyObjectFromDatabase(true);
			}
			...
			if (inventory->transferObject(darkRobe, -1)) {
				inventory->broadcastObject(darkRobe, true);
			} else {
				darkRobe->destroyObjectFromDatabase(true);
			}

		} else
		creature->sendSystemMessage("You must be aligned with a faction to start down the path to be a Jedi Knight.");
		return 0;		
	}
...
//MU--AH
//FUCK--AH
//playerObject.getSpentJediSkillPoints()
